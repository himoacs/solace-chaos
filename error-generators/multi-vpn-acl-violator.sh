...
while true; do
    ...
    # Try to access premium market data with restricted user (hourly test)
    bash "${SDKPERF_SCRIPT_PATH}" \
        ...
        -mr=1 -mn=3 -msa=256 >> logs/acl-violator.log 2>&1 &
    
    # Try to access admin trading functions with restricted user (hourly test)
    bash "${SDKPERF_SCRIPT_PATH}" \
        -cip="${SOLACE_BROKER_HOST}:${SOLACE_BROKER_PORT}" \
        -cu="${RESTRICTED_TRADE_USER}" \
        -cp="${RESTRICTED_TRADE_PASSWORD}" \
        -ptl="trading/admin/cancel-all-orders" \
        -mr=1 -mn=2 -msa=256 >> logs/acl-violator.log 2>&1 &
    
    # Try cross-VPN access without proper permissions (hourly test)
    bash "${SDKPERF_SCRIPT_PATH}" \
        -cip="${SOLACE_BROKER_HOST}:${SOLACE_BROKER_PORT}" \
        -cu="${RESTRICTED_MARKET_USER}" \
        -cp="${RESTRICTED_MARKET_PASSWORD}" \
        -ptl="trading/orders/equities/NYSE/new" \
        -mr=1 -mn=10 -msa=256 >> logs/acl-violator.log 2>&1 &
